...
const Card = ({ study, i, progress, range }: any) => {
  ...
  // Scale down the card from 1 to 0.9 as progress moves through its range
  const scale = useTransform(progress, range, [1, 0.9]);

  // Push the card slightly up and tilt it backward
  const yOffset = useTransform(progress, range, [0, -25]);
  const rotateX = useTransform(progress, range, [0, -6]); // 3D tilt

  const Icon = study.icon;
  const isLastCard = i === caseStudies.length - 1;

  return (
    <div
      className="h-auto py-6 md:py-0 md:h-[65vh] w-full flex justify-center relative md:sticky"
      style={isMobile ? {} : { top: "92px", zIndex: (i + 1) * 10 }}
    >
      <motion.div
        style={
          isMobile
            ? {}
            : {
                scale: isLastCard ? 1 : scale,
                y: isLastCard ? 0 : yOffset,
                rotateX: isLastCard ? 0 : rotateX,
        ...
        className="group relative rounded-[2rem] md:rounded-[2.5rem] bg-card border border-border/40 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.3)] dark:shadow-[0_30px_60px_rgba(0,0,0,0.6)] w-full h-full"
      >
        ...
        <div className="relative z-10 p-8 sm:p-12 flex flex-col md:flex-row gap-10 lg:gap-16 h-full">
...
const CaseStudiesSection = () => {
  ...
  return (
    ...
    <section
      ref={container}
      className="relative z-10 py-12 md:py-16 px-6 bg-background"
    >
      {/* Background Ambience */}
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-2 md:mb-12">
          ...
          >
            ...
            >
              Real Impact,{" "}
              <span className="text-gradient-cyan drop-shadow-[0_0_15px_rgba(0,255,255,0.3)] block">
                Proven Results.
              </span>
            </h2>
          </motion.div>
          ...
          >
            Don't just take our word for it. See how we’ve helped businesses win
            big, scale faster, and solve complex challenges.
          </motion.p>
        </div>
        ...
        <div className="relative w-full">
          {caseStudies.map((study, i) => {
            const startRange = i * (1 / caseStudies.length);
            const endRange = (i + 1) * (1 / caseStudies.length);
            return (
              <Card
                key={study.id}
                i={i}
